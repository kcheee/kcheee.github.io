---
layout: post
title: programmers 2단계 주차 요금 계산
categories : programmers 2단계
tag : [programmers,map,tuple,get]
---
...
카카오 문제는 항상 오래걸리는거 같다 ㅜㅜ. 이 문제도 거의 한시간 반 정도 걸려서 풀었다.ㅠㅜㅜ 막상 다 풀고 코드를 보면 이렇게 오래 걸릴 문제들이 아닌데 항상 오래걸리는거 같다,,
   
1. map과 튜플을 사용하였다. key=차량번호 value는 tuple을 사용하여 시간,내역,누적 시간으로 계산하였다.   
2. get을 사용하여 튜플안에 있는 값에 접근하였고, "out"이 있을 때 누적시간을 계산하였음
3. 계산하여 answer에 푸시
